...
Generated by the `skill-gen` Dagger module. Do not hand-edit files in this directory — re-running the generator overwrites everything under `pg-shop/`. Keep the `.env` files and `.gitignore` rules below at the project root (not inside this directory) so regeneration leaves them alone.
...
cp pg-shop/scripts/.env.example .env
...
#   cp pg-shop/scripts/.env.example .env.prod
...
```bash
op run --env-file=postgres.env -- bash pg-shop/scripts/query.sh "SELECT 1"
```
...
All commands below assume your shell is at the project root, so the wrapper is invoked by its full path (`pg-shop/scripts/query.sh`). The `query.sh` resolution order picks up `./.env` from the same project root.

### Smoke test

```bash
bash pg-shop/scripts/query.sh "SELECT 1"
```
...
```bash
bash pg-shop/scripts/query.sh < script.sql
```

Useful for `.sql` files you already have on disk, multi-line SQL that's awkward to inline as a single shell argument, and SQL that mixes plain statements with psql meta-commands (`\copy`, `\set`, `\timing`, `\d`, …). Plain multi-statement SQL works fine via the inline `-c` form too — `bash pg-shop/scripts/query.sh "SELECT 1; SELECT 2"` runs both statements in a single transaction — but `-c` can't mix SQL with psql meta-commands, so anything using `\copy` or similar has to come in via stdin.
...
bash pg-shop/scripts/query.sh "SELECT now()"
```
