// Code generated by dagger. DO NOT EDIT.

package dagger

import (
	"context"
	"encoding/json"

	"github.com/dagger/querybuilder"
)

// Envoy is the top-level builder type. All component factories hang
// off of it.
type Envoy struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	id *ID
}

func (r *Envoy) WithGraphQLQuery(q *querybuilder.Selection) *Envoy {
	return &Envoy{
		query: q,
	}
}

// EnvoyClusterOpts contains options for Envoy.Cluster
type EnvoyClusterOpts struct {

	// Default: "STRICT_DNS"
	ClusterType string

	Upstream *EnvoyUpstreamSecurity
}

// Cluster builds a Cluster optionally configured with upstream
// TLS / mTLS via an UpstreamSecurity profile. clusterType defaults
// to "STRICT_DNS"; unknown values return a non-nil error. A nil or
// plaintext upstream produces the same plaintext cluster as before.
func (r *Envoy) Cluster(name string, opts ...EnvoyClusterOpts) *EnvoyCluster {
	q := r.query.Select("cluster")
	for i := len(opts) - 1; i >= 0; i-- {
		// `clusterType` optional argument
		if !querybuilder.IsZeroValue(opts[i].ClusterType) {
			q = q.Arg("clusterType", opts[i].ClusterType)
		}
		// `upstream` optional argument
		if !querybuilder.IsZeroValue(opts[i].Upstream) {
			q = q.Arg("upstream", opts[i].Upstream)
		}
	}
	q = q.Arg("name", name)

	return &EnvoyCluster{
		query: q,
	}
}

// CustomHttpFilter builds an HTTP filter whose typed_config body is
// the caller-supplied YAML, spliced verbatim. yamlBody is validated
// via yaml.Unmarshal at construction time.
func (r *Envoy) CustomHTTPFilter(name string, yamlBody string) *EnvoyHTTPFilter {
	q := r.query.Select("customHttpFilter")
	q = q.Arg("name", name)
	q = q.Arg("yamlBody", yamlBody)

	return &EnvoyHTTPFilter{
		query: q,
	}
}

// CustomListener builds a Listener whose body is the caller-supplied
// YAML, spliced verbatim under static_resources.listeners with `name`
// keyed in by the builder. The yamlBody must NOT include a top-level
// `name:` key (the builder splices that from the argument).
func (r *Envoy) CustomListener(name string, yamlBody string) *EnvoyListener {
	q := r.query.Select("customListener")
	q = q.Arg("name", name)
	q = q.Arg("yamlBody", yamlBody)

	return &EnvoyListener{
		query: q,
	}
}

// Endpoint builds an Endpoint, validating host (non-empty) and port
// (1..65535).
func (r *Envoy) Endpoint(host string, port int) *EnvoyEndpoint {
	q := r.query.Select("endpoint")
	q = q.Arg("host", host)
	q = q.Arg("port", port)

	return &EnvoyEndpoint{
		query: q,
	}
}

// HttpConnectionManager builds an HCM bound to routeConfig.
func (r *Envoy) HTTPConnectionManager(statPrefix string, routeConfig *EnvoyRouteConfig) *EnvoyHTTPConnectionManager {
	assertNotNil("routeConfig", routeConfig)
	q := r.query.Select("httpConnectionManager")
	q = q.Arg("statPrefix", statPrefix)
	q = q.Arg("routeConfig", routeConfig)

	return &EnvoyHTTPConnectionManager{
		query: q,
	}
}

// EnvoyHTTPListenerOpts contains options for Envoy.HTTPListener
type EnvoyHTTPListenerOpts struct {

	// Default: "0.0.0.0"
	Address string

	Security *EnvoyServerSecurity
}

// HttpListener builds an L7 listener bound at address:port whose
// filter chain delegates to hcm. A nil or plaintext security profile
// renders the existing plaintext listener; TLS / MTLS mints a
// per-listener server leaf certificate from the supplied CA at
// factory time and embeds the transport_socket block in the filter
// chain.
func (r *Envoy) HTTPListener(name string, port int, hcm *EnvoyHTTPConnectionManager, opts ...EnvoyHTTPListenerOpts) *EnvoyListener {
	assertNotNil("hcm", hcm)
	q := r.query.Select("httpListener")
	for i := len(opts) - 1; i >= 0; i-- {
		// `address` optional argument
		if !querybuilder.IsZeroValue(opts[i].Address) {
			q = q.Arg("address", opts[i].Address)
		}
		// `security` optional argument
		if !querybuilder.IsZeroValue(opts[i].Security) {
			q = q.Arg("security", opts[i].Security)
		}
	}
	q = q.Arg("name", name)
	q = q.Arg("port", port)
	q = q.Arg("hcm", hcm)

	return &EnvoyListener{
		query: q,
	}
}

// A unique identifier for this Envoy.
func (r *Envoy) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *Envoy) XXX_GraphQLType() string {
	return "Envoy"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *Envoy) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *Envoy) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *Envoy) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *Envoy) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = Envoy{query: selectNode(dag.query, id, "Envoy")}
	return nil
}

// MtlsServerSecurity returns a ServerSecurity profile that terminates
// mTLS on the listener. caKeyStore signs the per-listener server
// leaf; clientTrustStore holds the CA(s) the listener will accept
// incoming client certs from (asymmetric trust is allowed).
func (r *Envoy) MtlsServerSecurity(caKeyStore *File, caKeyStorePassword *Secret, clientTrustStore *File, clientTrustStorePassword *Secret) *EnvoyServerSecurity {
	assertNotNil("caKeyStore", caKeyStore)
	assertNotNil("caKeyStorePassword", caKeyStorePassword)
	assertNotNil("clientTrustStore", clientTrustStore)
	assertNotNil("clientTrustStorePassword", clientTrustStorePassword)
	q := r.query.Select("mtlsServerSecurity")
	q = q.Arg("caKeyStore", caKeyStore)
	q = q.Arg("caKeyStorePassword", caKeyStorePassword)
	q = q.Arg("clientTrustStore", clientTrustStore)
	q = q.Arg("clientTrustStorePassword", clientTrustStorePassword)

	return &EnvoyServerSecurity{
		query: q,
	}
}

// MtlsUpstreamSecurity returns an UpstreamSecurity profile that opens
// an mTLS connection to upstreams: the upstream's server cert is
// validated against trustStore, and Envoy presents its own client
// leaf from keyStore.
func (r *Envoy) MtlsUpstreamSecurity(keyStore *File, keyStorePassword *Secret, trustStore *File, trustStorePassword *Secret) *EnvoyUpstreamSecurity {
	assertNotNil("keyStore", keyStore)
	assertNotNil("keyStorePassword", keyStorePassword)
	assertNotNil("trustStore", trustStore)
	assertNotNil("trustStorePassword", trustStorePassword)
	q := r.query.Select("mtlsUpstreamSecurity")
	q = q.Arg("keyStore", keyStore)
	q = q.Arg("keyStorePassword", keyStorePassword)
	q = q.Arg("trustStore", trustStore)
	q = q.Arg("trustStorePassword", trustStorePassword)

	return &EnvoyUpstreamSecurity{
		query: q,
	}
}

// PlaintextServerSecurity returns a ServerSecurity profile configured
// for unencrypted, unauthenticated traffic.
func (r *Envoy) PlaintextServerSecurity() *EnvoyServerSecurity {
	q := r.query.Select("plaintextServerSecurity")

	return &EnvoyServerSecurity{
		query: q,
	}
}

// PlaintextUpstreamSecurity returns an UpstreamSecurity profile
// configured for unencrypted, unauthenticated cluster traffic.
func (r *Envoy) PlaintextUpstreamSecurity() *EnvoyUpstreamSecurity {
	q := r.query.Select("plaintextUpstreamSecurity")

	return &EnvoyUpstreamSecurity{
		query: q,
	}
}

// EnvoyProxyOpts contains options for Envoy.Proxy
type EnvoyProxyOpts struct {

	// Default: "docker.io"
	Registry string

	// Default: "v1.32.1"
	Tag string

	// Default: 9901
	AdminPort int

	ConfigFile *File
}

// Proxy returns a Proxy backed by the envoyproxy/envoy image at
// <registry>/envoyproxy/envoy:<tag>. configFile, when supplied, fully
// replaces the rendered bootstrap; clusters added via WithCluster are
// ignored when an override is set, and listeners added via WithListener
// are ignored for rendering but still drive container port exposure so
// callers can expose ports from an override config.
func (r *Envoy) Proxy(opts ...EnvoyProxyOpts) *EnvoyProxy {
	q := r.query.Select("proxy")
	for i := len(opts) - 1; i >= 0; i-- {
		// `registry` optional argument
		if !querybuilder.IsZeroValue(opts[i].Registry) {
			q = q.Arg("registry", opts[i].Registry)
		}
		// `tag` optional argument
		if !querybuilder.IsZeroValue(opts[i].Tag) {
			q = q.Arg("tag", opts[i].Tag)
		}
		// `adminPort` optional argument
		if !querybuilder.IsZeroValue(opts[i].AdminPort) {
			q = q.Arg("adminPort", opts[i].AdminPort)
		}
		// `configFile` optional argument
		if !querybuilder.IsZeroValue(opts[i].ConfigFile) {
			q = q.Arg("configFile", opts[i].ConfigFile)
		}
	}

	return &EnvoyProxy{
		query: q,
	}
}

// RouteConfig builds an empty RouteConfig.
func (r *Envoy) RouteConfig(name string) *EnvoyRouteConfig {
	q := r.query.Select("routeConfig")
	q = q.Arg("name", name)

	return &EnvoyRouteConfig{
		query: q,
	}
}

// RoutePrefix builds a route matching paths with the given prefix and
// forwarding to cluster.
func (r *Envoy) RoutePrefix(prefix string, cluster string) *EnvoyRoute {
	q := r.query.Select("routePrefix")
	q = q.Arg("prefix", prefix)
	q = q.Arg("cluster", cluster)

	return &EnvoyRoute{
		query: q,
	}
}

// RouterHttpFilter returns the terminal envoy.filters.http.router
// filter. Per the Envoy contract this must be the last filter in the
// http_filters chain; the builder does NOT enforce ordering — Envoy
// rejects the bootstrap at startup if violated.
func (r *Envoy) RouterHTTPFilter() *EnvoyHTTPFilter {
	q := r.query.Select("routerHttpFilter")

	return &EnvoyHTTPFilter{
		query: q,
	}
}

// EnvoyTCPListenerOpts contains options for Envoy.TCPListener
type EnvoyTCPListenerOpts struct {

	// Default: "0.0.0.0"
	Address string

	Security *EnvoyServerSecurity
}

// TcpListener builds an L4 listener bound at address:port whose
// filter chain delegates to proxy. Accepts the same *ServerSecurity
// union as HttpListener — TLS / mTLS terminates on the listener and
// the resulting transport_socket lives on the filter chain alongside
// the tcp_proxy filter.
func (r *Envoy) TCPListener(name string, port int, proxy *EnvoyTCPProxy, opts ...EnvoyTCPListenerOpts) *EnvoyListener {
	assertNotNil("proxy", proxy)
	q := r.query.Select("tcpListener")
	for i := len(opts) - 1; i >= 0; i-- {
		// `address` optional argument
		if !querybuilder.IsZeroValue(opts[i].Address) {
			q = q.Arg("address", opts[i].Address)
		}
		// `security` optional argument
		if !querybuilder.IsZeroValue(opts[i].Security) {
			q = q.Arg("security", opts[i].Security)
		}
	}
	q = q.Arg("name", name)
	q = q.Arg("port", port)
	q = q.Arg("proxy", proxy)

	return &EnvoyListener{
		query: q,
	}
}

// TcpProxy builds a TcpProxy network filter targeting cluster.
func (r *Envoy) TCPProxy(statPrefix string, cluster string) *EnvoyTCPProxy {
	q := r.query.Select("tcpProxy")
	q = q.Arg("statPrefix", statPrefix)
	q = q.Arg("cluster", cluster)

	return &EnvoyTCPProxy{
		query: q,
	}
}

// TlsServerSecurity returns a ServerSecurity profile that terminates
// TLS on the listener. caKeyStore is a PKCS#12 archive containing the
// CA cert + private key used to mint a per-listener leaf certificate.
func (r *Envoy) TLSServerSecurity(caKeyStore *File, caKeyStorePassword *Secret) *EnvoyServerSecurity {
	assertNotNil("caKeyStore", caKeyStore)
	assertNotNil("caKeyStorePassword", caKeyStorePassword)
	q := r.query.Select("tlsServerSecurity")
	q = q.Arg("caKeyStore", caKeyStore)
	q = q.Arg("caKeyStorePassword", caKeyStorePassword)

	return &EnvoyServerSecurity{
		query: q,
	}
}

// TlsUpstreamSecurity returns an UpstreamSecurity profile that opens
// a TLS connection to upstreams. trustStore is a PKCS#12 archive of
// the CA(s) used to verify the upstream's server cert.
func (r *Envoy) TLSUpstreamSecurity(trustStore *File, trustStorePassword *Secret) *EnvoyUpstreamSecurity {
	assertNotNil("trustStore", trustStore)
	assertNotNil("trustStorePassword", trustStorePassword)
	q := r.query.Select("tlsUpstreamSecurity")
	q = q.Arg("trustStore", trustStore)
	q = q.Arg("trustStorePassword", trustStorePassword)

	return &EnvoyUpstreamSecurity{
		query: q,
	}
}

// VirtualHost builds an empty VirtualHost.
func (r *Envoy) VirtualHost(name string, domains []string) *EnvoyVirtualHost {
	q := r.query.Select("virtualHost")
	q = q.Arg("name", name)
	q = q.Arg("domains", domains)

	return &EnvoyVirtualHost{
		query: q,
	}
}

// XdsResources returns an empty discovery-resource set. Feed it
// listeners and clusters, then hand Directory() to
// (*Proxy).WithDynamicResources.
func (r *Envoy) XdsResources() *EnvoyXdsResources {
	q := r.query.Select("xdsResources")

	return &EnvoyXdsResources{
		query: q,
	}
}

// Cluster is a named upstream cluster of Endpoints, optionally
// configured with TLS / mTLS to the upstream side.
type EnvoyCluster struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	id   *ID
	kind *string
	name *string
}
type WithEnvoyClusterFunc func(r *EnvoyCluster) *EnvoyCluster

// With calls the provided function with current EnvoyCluster.
//
// This is useful for reusability and readability by not breaking the calling chain.
func (r *EnvoyCluster) With(f WithEnvoyClusterFunc) *EnvoyCluster {
	return f(r)
}

func (r *EnvoyCluster) WithGraphQLQuery(q *querybuilder.Selection) *EnvoyCluster {
	return &EnvoyCluster{
		query: q,
	}
}

func (r *EnvoyCluster) Endpoints(ctx context.Context) ([]EnvoyEndpoint, error) {
	q := r.query.Select("endpoints")

	q = q.Select("id")

	type endpoints struct {
		Id ID
	}

	convert := func(fields []endpoints) []EnvoyEndpoint {
		out := []EnvoyEndpoint{}

		for i := range fields {
			val := EnvoyEndpoint{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "EnvoyEndpoint")
			out = append(out, val)
		}

		return out
	}
	var response []endpoints

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

// A unique identifier for this EnvoyCluster.
func (r *EnvoyCluster) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *EnvoyCluster) XXX_GraphQLType() string {
	return "EnvoyCluster"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *EnvoyCluster) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *EnvoyCluster) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *EnvoyCluster) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *EnvoyCluster) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = EnvoyCluster{query: selectNode(dag.query, id, "EnvoyCluster")}
	return nil
}

func (r *EnvoyCluster) Kind(ctx context.Context) (string, error) {
	if r.kind != nil {
		return *r.kind, nil
	}
	q := r.query.Select("kind")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *EnvoyCluster) Name(ctx context.Context) (string, error) {
	if r.name != nil {
		return *r.name, nil
	}
	q := r.query.Select("name")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// WithEndpoint appends an Endpoint to the cluster and returns a new
// cluster value.
func (r *EnvoyCluster) WithEndpoint(ep *EnvoyEndpoint) *EnvoyCluster {
	assertNotNil("ep", ep)
	q := r.query.Select("withEndpoint")
	q = q.Arg("ep", ep)

	return &EnvoyCluster{
		query: q,
	}
}

// Endpoint is a single upstream address (host + port) that a Cluster
// resolves to.
type EnvoyEndpoint struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	host *string
	id   *ID
	port *int
}

func (r *EnvoyEndpoint) WithGraphQLQuery(q *querybuilder.Selection) *EnvoyEndpoint {
	return &EnvoyEndpoint{
		query: q,
	}
}

func (r *EnvoyEndpoint) Host(ctx context.Context) (string, error) {
	if r.host != nil {
		return *r.host, nil
	}
	q := r.query.Select("host")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// A unique identifier for this EnvoyEndpoint.
func (r *EnvoyEndpoint) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *EnvoyEndpoint) XXX_GraphQLType() string {
	return "EnvoyEndpoint"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *EnvoyEndpoint) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *EnvoyEndpoint) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *EnvoyEndpoint) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *EnvoyEndpoint) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = EnvoyEndpoint{query: selectNode(dag.query, id, "EnvoyEndpoint")}
	return nil
}

func (r *EnvoyEndpoint) Port(ctx context.Context) (int, error) {
	if r.port != nil {
		return *r.port, nil
	}
	q := r.query.Select("port")

	var response int

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// HttpConnectionManager is the L7 network filter that decodes HTTP
// frames, applies an ordered filter chain, and dispatches to a
// route_config.
type EnvoyHTTPConnectionManager struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	id         *ID
	statPrefix *string
}
type WithEnvoyHTTPConnectionManagerFunc func(r *EnvoyHTTPConnectionManager) *EnvoyHTTPConnectionManager

// With calls the provided function with current EnvoyHTTPConnectionManager.
//
// This is useful for reusability and readability by not breaking the calling chain.
func (r *EnvoyHTTPConnectionManager) With(f WithEnvoyHTTPConnectionManagerFunc) *EnvoyHTTPConnectionManager {
	return f(r)
}

func (r *EnvoyHTTPConnectionManager) WithGraphQLQuery(q *querybuilder.Selection) *EnvoyHTTPConnectionManager {
	return &EnvoyHTTPConnectionManager{
		query: q,
	}
}

func (r *EnvoyHTTPConnectionManager) HTTPFilters(ctx context.Context) ([]EnvoyHTTPFilter, error) {
	q := r.query.Select("httpFilters")

	q = q.Select("id")

	type httpFilters struct {
		Id ID
	}

	convert := func(fields []httpFilters) []EnvoyHTTPFilter {
		out := []EnvoyHTTPFilter{}

		for i := range fields {
			val := EnvoyHTTPFilter{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "EnvoyHttpFilter")
			out = append(out, val)
		}

		return out
	}
	var response []httpFilters

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

// A unique identifier for this EnvoyHttpConnectionManager.
func (r *EnvoyHTTPConnectionManager) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *EnvoyHTTPConnectionManager) XXX_GraphQLType() string {
	return "EnvoyHttpConnectionManager"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *EnvoyHTTPConnectionManager) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *EnvoyHTTPConnectionManager) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *EnvoyHTTPConnectionManager) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *EnvoyHTTPConnectionManager) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = EnvoyHTTPConnectionManager{query: selectNode(dag.query, id, "EnvoyHttpConnectionManager")}
	return nil
}

func (r *EnvoyHTTPConnectionManager) RouteConfig() *EnvoyRouteConfig {
	q := r.query.Select("routeConfig")

	return &EnvoyRouteConfig{
		query: q,
	}
}

func (r *EnvoyHTTPConnectionManager) StatPrefix(ctx context.Context) (string, error) {
	if r.statPrefix != nil {
		return *r.statPrefix, nil
	}
	q := r.query.Select("statPrefix")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// WithHttpFilter appends an HTTP filter to the HCM.
func (r *EnvoyHTTPConnectionManager) WithHTTPFilter(f *EnvoyHTTPFilter) *EnvoyHTTPConnectionManager {
	assertNotNil("f", f)
	q := r.query.Select("withHttpFilter")
	q = q.Arg("f", f)

	return &EnvoyHTTPConnectionManager{
		query: q,
	}
}

// HttpFilter is a single HTTP filter that participates in the
// HttpConnectionManager's filter chain. Body is the YAML body of the
// filter's `typed_config` map; for the terminal router filter Body
// is empty (the typed_config has a fixed shape and is filled in at
// render time).
type EnvoyHTTPFilter struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	body *string
	id   *ID
	name *string
}

func (r *EnvoyHTTPFilter) WithGraphQLQuery(q *querybuilder.Selection) *EnvoyHTTPFilter {
	return &EnvoyHTTPFilter{
		query: q,
	}
}

func (r *EnvoyHTTPFilter) Body(ctx context.Context) (string, error) {
	if r.body != nil {
		return *r.body, nil
	}
	q := r.query.Select("body")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// A unique identifier for this EnvoyHttpFilter.
func (r *EnvoyHTTPFilter) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *EnvoyHTTPFilter) XXX_GraphQLType() string {
	return "EnvoyHttpFilter"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *EnvoyHTTPFilter) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *EnvoyHTTPFilter) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *EnvoyHTTPFilter) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *EnvoyHTTPFilter) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = EnvoyHTTPFilter{query: selectNode(dag.query, id, "EnvoyHttpFilter")}
	return nil
}

func (r *EnvoyHTTPFilter) Name(ctx context.Context) (string, error) {
	if r.name != nil {
		return *r.name, nil
	}
	q := r.query.Select("name")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// Listener is a single Envoy listener — either typed
// (HttpListener/TcpListener) or opaque (CustomListener). Body is the
// YAML body for the listener excluding the top-level `name:` key,
// which is keyed in at render time from Name. ClusterRefs lists
// cluster names this listener references in its filter chain so
// (*Proxy).ConfigFile() can validate references against the
// registered cluster set; it is empty for CustomListener whose body
// is opaque.
type EnvoyListener struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	body *string
	id   *ID
	name *string
}

func (r *EnvoyListener) WithGraphQLQuery(q *querybuilder.Selection) *EnvoyListener {
	return &EnvoyListener{
		query: q,
	}
}

func (r *EnvoyListener) Body(ctx context.Context) (string, error) {
	if r.body != nil {
		return *r.body, nil
	}
	q := r.query.Select("body")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *EnvoyListener) ClusterRefs(ctx context.Context) ([]string, error) {
	q := r.query.Select("clusterRefs")

	var response []string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// A unique identifier for this EnvoyListener.
func (r *EnvoyListener) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *EnvoyListener) XXX_GraphQLType() string {
	return "EnvoyListener"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *EnvoyListener) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *EnvoyListener) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *EnvoyListener) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *EnvoyListener) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = EnvoyListener{query: selectNode(dag.query, id, "EnvoyListener")}
	return nil
}

func (r *EnvoyListener) Name(ctx context.Context) (string, error) {
	if r.name != nil {
		return *r.name, nil
	}
	q := r.query.Select("name")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// Proxy is a running Envoy instance with a composed (or
// caller-supplied) static-resources bootstrap, or — when
// DynamicResources is set — a file-based xDS bootstrap that
// discovers its listeners and clusters from a mounted directory.
type EnvoyProxy struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	adminEndpoint    *string
	adminPort        *int
	id               *ID
	listenerEndpoint *string
	registry         *string
	tag              *string
}
type WithEnvoyProxyFunc func(r *EnvoyProxy) *EnvoyProxy

// With calls the provided function with current EnvoyProxy.
//
// This is useful for reusability and readability by not breaking the calling chain.
func (r *EnvoyProxy) With(f WithEnvoyProxyFunc) *EnvoyProxy {
	return f(r)
}

func (r *EnvoyProxy) WithGraphQLQuery(q *querybuilder.Selection) *EnvoyProxy {
	return &EnvoyProxy{
		query: q,
	}
}

// AdminEndpoint returns host:adminPort for the running proxy.
func (r *EnvoyProxy) AdminEndpoint(ctx context.Context) (string, error) {
	if r.adminEndpoint != nil {
		return *r.adminEndpoint, nil
	}
	q := r.query.Select("adminEndpoint")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *EnvoyProxy) AdminPort(ctx context.Context) (int, error) {
	if r.adminPort != nil {
		return *r.adminPort, nil
	}
	q := r.query.Select("adminPort")

	var response int

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *EnvoyProxy) BindingHosts(ctx context.Context) ([]string, error) {
	q := r.query.Select("bindingHosts")

	var response []string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *EnvoyProxy) BindingSvcs(ctx context.Context) ([]Service, error) {
	q := r.query.Select("bindingSvcs")

	q = q.Select("id")

	type bindingSvcs struct {
		Id ID
	}

	convert := func(fields []bindingSvcs) []Service {
		out := []Service{}

		for i := range fields {
			val := Service{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "Service")
			out = append(out, val)
		}

		return out
	}
	var response []bindingSvcs

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

func (r *EnvoyProxy) Clusters(ctx context.Context) ([]EnvoyCluster, error) {
	q := r.query.Select("clusters")

	q = q.Select("id")

	type clusters struct {
		Id ID
	}

	convert := func(fields []clusters) []EnvoyCluster {
		out := []EnvoyCluster{}

		for i := range fields {
			val := EnvoyCluster{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "EnvoyCluster")
			out = append(out, val)
		}

		return out
	}
	var response []clusters

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

// ConfigFile returns the file that will be mounted as Envoy's -c
// argument: the caller-supplied override, the rendered
// static_resources bootstrap, or — when WithDynamicResources was
// called — the file-based xDS bootstrap. Returns a non-nil error if
// any listener references an unregistered cluster, if two listeners
// share a name, or if WithDynamicResources was mixed with any of
// WithListener / WithCluster / WithConfigFile.
func (r *EnvoyProxy) ConfigFile() *File {
	q := r.query.Select("configFile")

	return &File{
		query: q,
	}
}

// DynamicResources is the file-based xDS discovery-resource
// directory set via WithDynamicResources. Exclusive with
// Override / Listeners / Clusters.
func (r *EnvoyProxy) DynamicResources() *Directory {
	q := r.query.Select("dynamicResources")

	return &Directory{
		query: q,
	}
}

// A unique identifier for this EnvoyProxy.
func (r *EnvoyProxy) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *EnvoyProxy) XXX_GraphQLType() string {
	return "EnvoyProxy"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *EnvoyProxy) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *EnvoyProxy) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *EnvoyProxy) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *EnvoyProxy) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = EnvoyProxy{query: selectNode(dag.query, id, "EnvoyProxy")}
	return nil
}

// ListenerEndpoint returns host:port for the named listener on the
// running proxy — resolved against the registered Listeners, or
// against the mounted lds.yaml when the proxy runs in file-based xDS
// mode. Returns a non-nil error if no listener matches name or if the
// listener has no recognizable socket_address.port_value.
func (r *EnvoyProxy) ListenerEndpoint(ctx context.Context, name string) (string, error) {
	if r.listenerEndpoint != nil {
		return *r.listenerEndpoint, nil
	}
	q := r.query.Select("listenerEndpoint")
	q = q.Arg("name", name)

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *EnvoyProxy) Listeners(ctx context.Context) ([]EnvoyListener, error) {
	q := r.query.Select("listeners")

	q = q.Select("id")

	type listeners struct {
		Id ID
	}

	convert := func(fields []listeners) []EnvoyListener {
		out := []EnvoyListener{}

		for i := range fields {
			val := EnvoyListener{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "EnvoyListener")
			out = append(out, val)
		}

		return out
	}
	var response []listeners

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

func (r *EnvoyProxy) Override() *File {
	q := r.query.Select("override")

	return &File{
		query: q,
	}
}

func (r *EnvoyProxy) Registry(ctx context.Context) (string, error) {
	if r.registry != nil {
		return *r.registry, nil
	}
	q := r.query.Select("registry")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// Service returns the running Envoy container. Listens on
// AdminPort (admin) plus each registered listener's port — read from
// the mounted lds.yaml when the proxy runs in file-based xDS mode.
// When no override and no listeners/clusters are registered, launches
// with no `-c` flag so the envoy binary exits non-zero — exposed
// verbatim so callers can detect the misconfig via service-binding
// probes.
func (r *EnvoyProxy) Service() *Service {
	q := r.query.Select("service")

	return &Service{
		query: q,
	}
}

func (r *EnvoyProxy) Tag(ctx context.Context) (string, error) {
	if r.tag != nil {
		return *r.tag, nil
	}
	q := r.query.Select("tag")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// WithCluster appends a cluster to the proxy.
func (r *EnvoyProxy) WithCluster(c *EnvoyCluster) *EnvoyProxy {
	assertNotNil("c", c)
	q := r.query.Select("withCluster")
	q = q.Arg("c", c)

	return &EnvoyProxy{
		query: q,
	}
}

// WithConfigFile fully replaces the rendered bootstrap.
func (r *EnvoyProxy) WithConfigFile(f *File) *EnvoyProxy {
	assertNotNil("f", f)
	q := r.query.Select("withConfigFile")
	q = q.Arg("f", f)

	return &EnvoyProxy{
		query: q,
	}
}

// WithDynamicResources switches the proxy from a rendered
// static_resources bootstrap to file-based xDS: the bootstrap's
// dynamic_resources block points lds_config at
// /etc/envoy/xds/lds.yaml and cds_config at /etc/envoy/xds/cds.yaml,
// and dir is mounted at /etc/envoy/xds. dir must contain at minimum
// lds.yaml and cds.yaml, each a v3 discovery response; rds.yaml is
// loaded only when a listener's HttpConnectionManager references it
// via an `rds` config source (the bootstrap does not wire RDS
// itself).
//
// This mode is exclusive with WithListener / WithCluster /
// WithConfigFile: calling both makes ConfigFile() (and therefore
// Service()) return a non-nil error.
//
// Hot reload is NOT exercised. Dagger's WithMountedDirectory is
// content-addressed and immutable at mount time, so the mounted
// resource files never change for the lifetime of the container —
// only Envoy's initial discovery path runs. Callers wanting to test
// reload semantics need to launch a fresh Proxy per resource
// snapshot.
func (r *EnvoyProxy) WithDynamicResources(dir *Directory) *EnvoyProxy {
	assertNotNil("dir", dir)
	q := r.query.Select("withDynamicResources")
	q = q.Arg("dir", dir)

	return &EnvoyProxy{
		query: q,
	}
}

// WithListener appends a listener to the proxy.
func (r *EnvoyProxy) WithListener(l *EnvoyListener) *EnvoyProxy {
	assertNotNil("l", l)
	q := r.query.Select("withListener")
	q = q.Arg("l", l)

	return &EnvoyProxy{
		query: q,
	}
}

// WithServiceBinding binds an upstream service into Envoy's network
// so cluster endpoints can reach it by hostname.
func (r *EnvoyProxy) WithServiceBinding(host string, svc *Service) *EnvoyProxy {
	assertNotNil("svc", svc)
	q := r.query.Select("withServiceBinding")
	q = q.Arg("host", host)
	q = q.Arg("svc", svc)

	return &EnvoyProxy{
		query: q,
	}
}

// Route is a single HTTP route (prefix match → cluster).
type EnvoyRoute struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	cluster *string
	id      *ID
	prefix  *string
}

func (r *EnvoyRoute) WithGraphQLQuery(q *querybuilder.Selection) *EnvoyRoute {
	return &EnvoyRoute{
		query: q,
	}
}

func (r *EnvoyRoute) Cluster(ctx context.Context) (string, error) {
	if r.cluster != nil {
		return *r.cluster, nil
	}
	q := r.query.Select("cluster")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// A unique identifier for this EnvoyRoute.
func (r *EnvoyRoute) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *EnvoyRoute) XXX_GraphQLType() string {
	return "EnvoyRoute"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *EnvoyRoute) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *EnvoyRoute) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *EnvoyRoute) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *EnvoyRoute) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = EnvoyRoute{query: selectNode(dag.query, id, "EnvoyRoute")}
	return nil
}

func (r *EnvoyRoute) Prefix(ctx context.Context) (string, error) {
	if r.prefix != nil {
		return *r.prefix, nil
	}
	q := r.query.Select("prefix")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// RouteConfig is a named route configuration (a set of virtual hosts).
type EnvoyRouteConfig struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	id   *ID
	name *string
}
type WithEnvoyRouteConfigFunc func(r *EnvoyRouteConfig) *EnvoyRouteConfig

// With calls the provided function with current EnvoyRouteConfig.
//
// This is useful for reusability and readability by not breaking the calling chain.
func (r *EnvoyRouteConfig) With(f WithEnvoyRouteConfigFunc) *EnvoyRouteConfig {
	return f(r)
}

func (r *EnvoyRouteConfig) WithGraphQLQuery(q *querybuilder.Selection) *EnvoyRouteConfig {
	return &EnvoyRouteConfig{
		query: q,
	}
}

// A unique identifier for this EnvoyRouteConfig.
func (r *EnvoyRouteConfig) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *EnvoyRouteConfig) XXX_GraphQLType() string {
	return "EnvoyRouteConfig"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *EnvoyRouteConfig) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *EnvoyRouteConfig) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *EnvoyRouteConfig) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *EnvoyRouteConfig) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = EnvoyRouteConfig{query: selectNode(dag.query, id, "EnvoyRouteConfig")}
	return nil
}

func (r *EnvoyRouteConfig) Name(ctx context.Context) (string, error) {
	if r.name != nil {
		return *r.name, nil
	}
	q := r.query.Select("name")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *EnvoyRouteConfig) VirtualHosts(ctx context.Context) ([]EnvoyVirtualHost, error) {
	q := r.query.Select("virtualHosts")

	q = q.Select("id")

	type virtualHosts struct {
		Id ID
	}

	convert := func(fields []virtualHosts) []EnvoyVirtualHost {
		out := []EnvoyVirtualHost{}

		for i := range fields {
			val := EnvoyVirtualHost{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "EnvoyVirtualHost")
			out = append(out, val)
		}

		return out
	}
	var response []virtualHosts

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

// WithVirtualHost appends a VirtualHost to the route config.
func (r *EnvoyRouteConfig) WithVirtualHost(v *EnvoyVirtualHost) *EnvoyRouteConfig {
	assertNotNil("v", v)
	q := r.query.Select("withVirtualHost")
	q = q.Arg("v", v)

	return &EnvoyRouteConfig{
		query: q,
	}
}

// ServerSecurity describes how an Envoy listener authenticates and
// encrypts traffic from downstream clients. Plaintext is the default;
// TLS and mTLS modes carry the PKI material needed to terminate TLS
// on the listener and (for mTLS) verify client certificates.
type EnvoyServerSecurity struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	id *ID
}

func (r *EnvoyServerSecurity) WithGraphQLQuery(q *querybuilder.Selection) *EnvoyServerSecurity {
	return &EnvoyServerSecurity{
		query: q,
	}
}

// A unique identifier for this EnvoyServerSecurity.
func (r *EnvoyServerSecurity) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *EnvoyServerSecurity) XXX_GraphQLType() string {
	return "EnvoyServerSecurity"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *EnvoyServerSecurity) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *EnvoyServerSecurity) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *EnvoyServerSecurity) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *EnvoyServerSecurity) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = EnvoyServerSecurity{query: selectNode(dag.query, id, "EnvoyServerSecurity")}
	return nil
}

// TcpProxy is the network-level filter that forwards bytes from a
// TcpListener to a single cluster.
type EnvoyTCPProxy struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	cluster    *string
	id         *ID
	statPrefix *string
}

func (r *EnvoyTCPProxy) WithGraphQLQuery(q *querybuilder.Selection) *EnvoyTCPProxy {
	return &EnvoyTCPProxy{
		query: q,
	}
}

func (r *EnvoyTCPProxy) Cluster(ctx context.Context) (string, error) {
	if r.cluster != nil {
		return *r.cluster, nil
	}
	q := r.query.Select("cluster")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// A unique identifier for this EnvoyTcpProxy.
func (r *EnvoyTCPProxy) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *EnvoyTCPProxy) XXX_GraphQLType() string {
	return "EnvoyTcpProxy"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *EnvoyTCPProxy) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *EnvoyTCPProxy) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *EnvoyTCPProxy) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *EnvoyTCPProxy) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = EnvoyTCPProxy{query: selectNode(dag.query, id, "EnvoyTcpProxy")}
	return nil
}

func (r *EnvoyTCPProxy) StatPrefix(ctx context.Context) (string, error) {
	if r.statPrefix != nil {
		return *r.statPrefix, nil
	}
	q := r.query.Select("statPrefix")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// UpstreamSecurity describes how an Envoy cluster authenticates and
// encrypts traffic to upstream endpoints. Plaintext is the default;
// TLS validates the upstream's server cert; mTLS additionally
// presents a client leaf to the upstream.
type EnvoyUpstreamSecurity struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	id *ID
}

func (r *EnvoyUpstreamSecurity) WithGraphQLQuery(q *querybuilder.Selection) *EnvoyUpstreamSecurity {
	return &EnvoyUpstreamSecurity{
		query: q,
	}
}

// A unique identifier for this EnvoyUpstreamSecurity.
func (r *EnvoyUpstreamSecurity) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *EnvoyUpstreamSecurity) XXX_GraphQLType() string {
	return "EnvoyUpstreamSecurity"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *EnvoyUpstreamSecurity) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *EnvoyUpstreamSecurity) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *EnvoyUpstreamSecurity) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *EnvoyUpstreamSecurity) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = EnvoyUpstreamSecurity{query: selectNode(dag.query, id, "EnvoyUpstreamSecurity")}
	return nil
}

// VirtualHost is a named virtual host with a domain list and an
// ordered set of routes.
type EnvoyVirtualHost struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	id   *ID
	name *string
}
type WithEnvoyVirtualHostFunc func(r *EnvoyVirtualHost) *EnvoyVirtualHost

// With calls the provided function with current EnvoyVirtualHost.
//
// This is useful for reusability and readability by not breaking the calling chain.
func (r *EnvoyVirtualHost) With(f WithEnvoyVirtualHostFunc) *EnvoyVirtualHost {
	return f(r)
}

func (r *EnvoyVirtualHost) WithGraphQLQuery(q *querybuilder.Selection) *EnvoyVirtualHost {
	return &EnvoyVirtualHost{
		query: q,
	}
}

func (r *EnvoyVirtualHost) Domains(ctx context.Context) ([]string, error) {
	q := r.query.Select("domains")

	var response []string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// A unique identifier for this EnvoyVirtualHost.
func (r *EnvoyVirtualHost) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *EnvoyVirtualHost) XXX_GraphQLType() string {
	return "EnvoyVirtualHost"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *EnvoyVirtualHost) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *EnvoyVirtualHost) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *EnvoyVirtualHost) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *EnvoyVirtualHost) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = EnvoyVirtualHost{query: selectNode(dag.query, id, "EnvoyVirtualHost")}
	return nil
}

func (r *EnvoyVirtualHost) Name(ctx context.Context) (string, error) {
	if r.name != nil {
		return *r.name, nil
	}
	q := r.query.Select("name")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *EnvoyVirtualHost) Routes(ctx context.Context) ([]EnvoyRoute, error) {
	q := r.query.Select("routes")

	q = q.Select("id")

	type routes struct {
		Id ID
	}

	convert := func(fields []routes) []EnvoyRoute {
		out := []EnvoyRoute{}

		for i := range fields {
			val := EnvoyRoute{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "EnvoyRoute")
			out = append(out, val)
		}

		return out
	}
	var response []routes

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

// WithRoute appends a route to the virtual host.
func (r *EnvoyVirtualHost) WithRoute(route *EnvoyRoute) *EnvoyVirtualHost {
	assertNotNil("route", route)
	q := r.query.Select("withRoute")
	q = q.Arg("route", route)

	return &EnvoyVirtualHost{
		query: q,
	}
}

// XdsResources composes the same v1 component types the static
// builders take (Listener, Cluster) into the discovery-resource files
// a file-based xDS Proxy consumes. Composition mirrors Proxy: WithX
// methods return shallow copies.
type EnvoyXdsResources struct { // envoy (../../../../:0:0)
	query *querybuilder.Selection

	id *ID
}
type WithEnvoyXdsResourcesFunc func(r *EnvoyXdsResources) *EnvoyXdsResources

// With calls the provided function with current EnvoyXdsResources.
//
// This is useful for reusability and readability by not breaking the calling chain.
func (r *EnvoyXdsResources) With(f WithEnvoyXdsResourcesFunc) *EnvoyXdsResources {
	return f(r)
}

func (r *EnvoyXdsResources) WithGraphQLQuery(q *querybuilder.Selection) *EnvoyXdsResources {
	return &EnvoyXdsResources{
		query: q,
	}
}

func (r *EnvoyXdsResources) Clusters(ctx context.Context) ([]EnvoyCluster, error) {
	q := r.query.Select("clusters")

	q = q.Select("id")

	type clusters struct {
		Id ID
	}

	convert := func(fields []clusters) []EnvoyCluster {
		out := []EnvoyCluster{}

		for i := range fields {
			val := EnvoyCluster{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "EnvoyCluster")
			out = append(out, val)
		}

		return out
	}
	var response []clusters

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

// Directory renders lds.yaml and cds.yaml — v3 discovery responses
// carrying the registered listeners and clusters — into a directory
// suitable for (*Proxy).WithDynamicResources. Each resource is
// structurally identical to the static_resources entry the same
// component produces in static mode, plus the `@type` discriminator
// the discovery response requires.
//
// Route configurations stay inline in each listener's
// HttpConnectionManager, so no rds.yaml is emitted: the v1 builders
// have no way to express an RDS config source. Callers who need RDS
// bring their own directory containing rds.yaml plus listeners that
// reference it.
//
// Returns a non-nil error under the same conditions as
// (*Proxy).ConfigFile() — a listener referencing a cluster that isn't
// registered here, or two listeners sharing a name — and also when
// any component carries TLS / mTLS security, whose key material only
// (*Proxy).Service() can mount and which the Proxy cannot see through
// an opaque resource directory.
func (r *EnvoyXdsResources) Directory() *Directory {
	q := r.query.Select("directory")

	return &Directory{
		query: q,
	}
}

// A unique identifier for this EnvoyXdsResources.
func (r *EnvoyXdsResources) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *EnvoyXdsResources) XXX_GraphQLType() string {
	return "EnvoyXdsResources"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *EnvoyXdsResources) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *EnvoyXdsResources) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *EnvoyXdsResources) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *EnvoyXdsResources) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = EnvoyXdsResources{query: selectNode(dag.query, id, "EnvoyXdsResources")}
	return nil
}

func (r *EnvoyXdsResources) Listeners(ctx context.Context) ([]EnvoyListener, error) {
	q := r.query.Select("listeners")

	q = q.Select("id")

	type listeners struct {
		Id ID
	}

	convert := func(fields []listeners) []EnvoyListener {
		out := []EnvoyListener{}

		for i := range fields {
			val := EnvoyListener{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "EnvoyListener")
			out = append(out, val)
		}

		return out
	}
	var response []listeners

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

// WithCluster appends a cluster to the CDS resource set.
func (r *EnvoyXdsResources) WithCluster(c *EnvoyCluster) *EnvoyXdsResources {
	assertNotNil("c", c)
	q := r.query.Select("withCluster")
	q = q.Arg("c", c)

	return &EnvoyXdsResources{
		query: q,
	}
}

// WithListener appends a listener to the LDS resource set.
func (r *EnvoyXdsResources) WithListener(l *EnvoyListener) *EnvoyXdsResources {
	assertNotNil("l", l)
	q := r.query.Select("withListener")
	q = q.Arg("l", l)

	return &EnvoyXdsResources{
		query: q,
	}
}

func (r *Query) Envoy() *Envoy { // envoy (../../../../:0:0)
	q := r.query.Select("envoy")

	return &Envoy{
		query: q,
	}
}
