// Code generated by dagger. DO NOT EDIT.

package dagger

import (
	"context"
	"encoding/json"

	"github.com/dagger/querybuilder"
)

type Otel struct { // otel (../../../../:0:0)
	query *querybuilder.Selection

	id *ID
}

func (r *Otel) WithGraphQLQuery(q *querybuilder.Selection) *Otel {
	return &Otel{
		query: q,
	}
}

// BatchProcessor builds a batch processor with collector defaults.
func (r *Otel) BatchProcessor(name string) *OtelProcessor {
	q := r.query.Select("batchProcessor")
	q = q.Arg("name", name)

	return &OtelProcessor{
		query: q,
	}
}

// OtelContribOpts contains options for Otel.Contrib
type OtelContribOpts struct {

	// Default: "docker.io"
	Registry string

	// Default: "0.130.1"
	Tag string

	ConfigFile *File
}

// Contrib returns a ContribCollector backed by the
// otel/opentelemetry-collector-contrib image. See Core.
func (r *Otel) Contrib(opts ...OtelContribOpts) *OtelContribCollector {
	q := r.query.Select("contrib")
	for i := len(opts) - 1; i >= 0; i-- {
		// `registry` optional argument
		if !querybuilder.IsZeroValue(opts[i].Registry) {
			q = q.Arg("registry", opts[i].Registry)
		}
		// `tag` optional argument
		if !querybuilder.IsZeroValue(opts[i].Tag) {
			q = q.Arg("tag", opts[i].Tag)
		}
		// `configFile` optional argument
		if !querybuilder.IsZeroValue(opts[i].ConfigFile) {
			q = q.Arg("configFile", opts[i].ConfigFile)
		}
	}

	return &OtelContribCollector{
		query: q,
	}
}

// OtelCoreOpts contains options for Otel.Core
type OtelCoreOpts struct {

	// Default: "docker.io"
	Registry string

	// Default: "0.130.1"
	Tag string

	ConfigFile *File
}

// Core returns a CoreCollector backed by the
// otel/opentelemetry-collector image at <registry>/<image>:<tag>.
// configFile, when supplied, fully replaces the rendered pipeline
// YAML; the image path is fixed.
func (r *Otel) Core(opts ...OtelCoreOpts) *OtelCoreCollector {
	q := r.query.Select("core")
	for i := len(opts) - 1; i >= 0; i-- {
		// `registry` optional argument
		if !querybuilder.IsZeroValue(opts[i].Registry) {
			q = q.Arg("registry", opts[i].Registry)
		}
		// `tag` optional argument
		if !querybuilder.IsZeroValue(opts[i].Tag) {
			q = q.Arg("tag", opts[i].Tag)
		}
		// `configFile` optional argument
		if !querybuilder.IsZeroValue(opts[i].ConfigFile) {
			q = q.Arg("configFile", opts[i].ConfigFile)
		}
	}

	return &OtelCoreCollector{
		query: q,
	}
}

// CustomExporter — see CustomReceiver.
func (r *Otel) CustomExporter(kind string, name string, yamlBody string) *OtelExporter {
	q := r.query.Select("customExporter")
	q = q.Arg("kind", kind)
	q = q.Arg("name", name)
	q = q.Arg("yamlBody", yamlBody)

	return &OtelExporter{
		query: q,
	}
}

// CustomProcessor — see CustomReceiver.
func (r *Otel) CustomProcessor(kind string, name string, yamlBody string) *OtelProcessor {
	q := r.query.Select("customProcessor")
	q = q.Arg("kind", kind)
	q = q.Arg("name", name)
	q = q.Arg("yamlBody", yamlBody)

	return &OtelProcessor{
		query: q,
	}
}

// CustomReceiver builds a receiver of arbitrary kind whose body is the
// caller-supplied YAML, spliced verbatim under `receivers.<kind>/<name>`.
func (r *Otel) CustomReceiver(kind string, name string, yamlBody string) *OtelReceiver {
	q := r.query.Select("customReceiver")
	q = q.Arg("kind", kind)
	q = q.Arg("name", name)
	q = q.Arg("yamlBody", yamlBody)

	return &OtelReceiver{
		query: q,
	}
}

// DebugExporter builds the stdout `debug` exporter at verbosity=detailed.
func (r *Otel) DebugExporter(name string) *OtelExporter {
	q := r.query.Select("debugExporter")
	q = q.Arg("name", name)

	return &OtelExporter{
		query: q,
	}
}

// DebugPipeline is a pre-wired smoke-test pipeline of
// otlp receiver → batch processor → debug exporter for signal.
// Component names are fixed (`otlp/debug`, `batch/debug`,
// `debug/debug`); the pipeline name is `debug`.
func (r *Otel) DebugPipeline(signal string) *OtelPipeline {
	q := r.query.Select("debugPipeline")
	q = q.Arg("signal", signal)

	return &OtelPipeline{
		query: q,
	}
}

// A unique identifier for this Otel.
func (r *Otel) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *Otel) XXX_GraphQLType() string {
	return "Otel"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *Otel) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *Otel) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *Otel) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *Otel) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = Otel{query: selectNode(dag.query, id, "Otel")}
	return nil
}

// MemoryLimiterProcessor builds a memory_limiter processor with
// conservative defaults (check_interval: 1s, limit_mib: 512). Callers
// needing different thresholds should reach for CustomProcessor.
func (r *Otel) MemoryLimiterProcessor(name string) *OtelProcessor {
	q := r.query.Select("memoryLimiterProcessor")
	q = q.Arg("name", name)

	return &OtelProcessor{
		query: q,
	}
}

// OtelOtlpExporterOpts contains options for Otel.OtlpExporter
type OtelOtlpExporterOpts struct {
	// PEM-encoded CA certificate to verify the receiver against. When set
	// the exporter speaks TLS instead of plaintext.
	CaCert *File
	// PEM-encoded client certificate presented for mTLS. Must be paired
	// with clientKey.
	ClientCert *File
	// PEM-encoded PKCS#8 client private key for mTLS. Must be paired with
	// clientCert.
	ClientKey *Secret
}

// OtlpExporter builds an OTLP gRPC exporter pointing at endpoint
// (host:port, no scheme). With no TLS options the exporter is plaintext
// (tls.insecure=true). Supplying caCert pins the server CA; supplying
// clientCert + clientKey (which must be given together) presents an mTLS
// identity.
func (r *Otel) OtlpExporter(name string, endpoint string, opts ...OtelOtlpExporterOpts) *OtelExporter {
	q := r.query.Select("otlpExporter")
	for i := len(opts) - 1; i >= 0; i-- {
		// `caCert` optional argument
		if !querybuilder.IsZeroValue(opts[i].CaCert) {
			q = q.Arg("caCert", opts[i].CaCert)
		}
		// `clientCert` optional argument
		if !querybuilder.IsZeroValue(opts[i].ClientCert) {
			q = q.Arg("clientCert", opts[i].ClientCert)
		}
		// `clientKey` optional argument
		if !querybuilder.IsZeroValue(opts[i].ClientKey) {
			q = q.Arg("clientKey", opts[i].ClientKey)
		}
	}
	q = q.Arg("name", name)
	q = q.Arg("endpoint", endpoint)

	return &OtelExporter{
		query: q,
	}
}

// OtelOtlpHTTPExporterOpts contains options for Otel.OtlpHTTPExporter
type OtelOtlpHTTPExporterOpts struct {
	CaCert *File

	ClientCert *File

	ClientKey *Secret
}

// OtlpHttpExporter builds an OTLP/HTTP exporter pointing at endpoint
// (URL with scheme, e.g. http://loki:3100/otlp). TLS options behave as
// on OtlpExporter; point endpoint at an https:// URL when supplying them.
func (r *Otel) OtlpHTTPExporter(name string, endpoint string, opts ...OtelOtlpHTTPExporterOpts) *OtelExporter {
	q := r.query.Select("otlpHttpExporter")
	for i := len(opts) - 1; i >= 0; i-- {
		// `caCert` optional argument
		if !querybuilder.IsZeroValue(opts[i].CaCert) {
			q = q.Arg("caCert", opts[i].CaCert)
		}
		// `clientCert` optional argument
		if !querybuilder.IsZeroValue(opts[i].ClientCert) {
			q = q.Arg("clientCert", opts[i].ClientCert)
		}
		// `clientKey` optional argument
		if !querybuilder.IsZeroValue(opts[i].ClientKey) {
			q = q.Arg("clientKey", opts[i].ClientKey)
		}
	}
	q = q.Arg("name", name)
	q = q.Arg("endpoint", endpoint)

	return &OtelExporter{
		query: q,
	}
}

// OtlpReceiver builds the standard OTLP receiver listening on gRPC :4317
// and HTTP :4318.
func (r *Otel) OtlpReceiver(name string) *OtelReceiver {
	q := r.query.Select("otlpReceiver")
	q = q.Arg("name", name)

	return &OtelReceiver{
		query: q,
	}
}

// Pipeline builds an empty pipeline for signal (logs|traces|metrics)
// keyed at <signal>/<name> in the rendered config.
func (r *Otel) Pipeline(signal string, name string) *OtelPipeline {
	q := r.query.Select("pipeline")
	q = q.Arg("signal", signal)
	q = q.Arg("name", name)

	return &OtelPipeline{
		query: q,
	}
}

// ResourceProcessor builds a no-op resource processor (empty
// attributes list). Callers needing actual attribute upserts should
// reach for CustomProcessor.
func (r *Otel) ResourceProcessor(name string) *OtelProcessor {
	q := r.query.Select("resourceProcessor")
	q = q.Arg("name", name)

	return &OtelProcessor{
		query: q,
	}
}

// ContribCollector wraps the otel/opentelemetry-collector-contrib
// image. Method set is identical to CoreCollector; only the image
// path differs, so the rendering and service helpers are shared.
type OtelContribCollector struct { // otel (../../../../:0:0)
	query *querybuilder.Selection

	id               *ID
	otlpGrpcEndpoint *string
	otlpHttpEndpoint *string
	registry         *string
	tag              *string
}
type WithOtelContribCollectorFunc func(r *OtelContribCollector) *OtelContribCollector

// With calls the provided function with current OtelContribCollector.
//
// This is useful for reusability and readability by not breaking the calling chain.
func (r *OtelContribCollector) With(f WithOtelContribCollectorFunc) *OtelContribCollector {
	return f(r)
}

func (r *OtelContribCollector) WithGraphQLQuery(q *querybuilder.Selection) *OtelContribCollector {
	return &OtelContribCollector{
		query: q,
	}
}

func (r *OtelContribCollector) BindingHosts(ctx context.Context) ([]string, error) {
	q := r.query.Select("bindingHosts")

	var response []string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *OtelContribCollector) BindingSvcs(ctx context.Context) ([]Service, error) {
	q := r.query.Select("bindingSvcs")

	q = q.Select("id")

	type bindingSvcs struct {
		Id ID
	}

	convert := func(fields []bindingSvcs) []Service {
		out := []Service{}

		for i := range fields {
			val := Service{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "Service")
			out = append(out, val)
		}

		return out
	}
	var response []bindingSvcs

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

// ConfigFile — see CoreCollector.ConfigFile.
func (r *OtelContribCollector) ConfigFile() *File {
	q := r.query.Select("configFile")

	return &File{
		query: q,
	}
}

// A unique identifier for this OtelContribCollector.
func (r *OtelContribCollector) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *OtelContribCollector) XXX_GraphQLType() string {
	return "OtelContribCollector"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *OtelContribCollector) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *OtelContribCollector) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *OtelContribCollector) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *OtelContribCollector) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = OtelContribCollector{query: selectNode(dag.query, id, "OtelContribCollector")}
	return nil
}

// OtlpGrpcEndpoint — see CoreCollector.OtlpGrpcEndpoint.
func (r *OtelContribCollector) OtlpGrpcEndpoint(ctx context.Context) (string, error) {
	if r.otlpGrpcEndpoint != nil {
		return *r.otlpGrpcEndpoint, nil
	}
	q := r.query.Select("otlpGrpcEndpoint")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// OtlpHttpEndpoint — see CoreCollector.OtlpHttpEndpoint.
func (r *OtelContribCollector) OtlpHTTPEndpoint(ctx context.Context) (string, error) {
	if r.otlpHttpEndpoint != nil {
		return *r.otlpHttpEndpoint, nil
	}
	q := r.query.Select("otlpHttpEndpoint")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *OtelContribCollector) Override() *File {
	q := r.query.Select("override")

	return &File{
		query: q,
	}
}

func (r *OtelContribCollector) Pipelines(ctx context.Context) ([]OtelPipeline, error) {
	q := r.query.Select("pipelines")

	q = q.Select("id")

	type pipelines struct {
		Id ID
	}

	convert := func(fields []pipelines) []OtelPipeline {
		out := []OtelPipeline{}

		for i := range fields {
			val := OtelPipeline{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "OtelPipeline")
			out = append(out, val)
		}

		return out
	}
	var response []pipelines

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

func (r *OtelContribCollector) Registry(ctx context.Context) (string, error) {
	if r.registry != nil {
		return *r.registry, nil
	}
	q := r.query.Select("registry")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// Service — see CoreCollector.Service.
func (r *OtelContribCollector) Service() *Service {
	q := r.query.Select("service")

	return &Service{
		query: q,
	}
}

func (r *OtelContribCollector) Tag(ctx context.Context) (string, error) {
	if r.tag != nil {
		return *r.tag, nil
	}
	q := r.query.Select("tag")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// WithConfigFile — see CoreCollector.WithConfigFile.
func (r *OtelContribCollector) WithConfigFile(f *File) *OtelContribCollector {
	assertNotNil("f", f)
	q := r.query.Select("withConfigFile")
	q = q.Arg("f", f)

	return &OtelContribCollector{
		query: q,
	}
}

// WithMtls — see CoreCollector.WithMtls.
func (r *OtelContribCollector) WithMtls(clientCa *File) *OtelContribCollector {
	assertNotNil("clientCa", clientCa)
	q := r.query.Select("withMtls")
	q = q.Arg("clientCa", clientCa)

	return &OtelContribCollector{
		query: q,
	}
}

// WithPipeline — see CoreCollector.WithPipeline.
func (r *OtelContribCollector) WithPipeline(p *OtelPipeline) *OtelContribCollector {
	assertNotNil("p", p)
	q := r.query.Select("withPipeline")
	q = q.Arg("p", p)

	return &OtelContribCollector{
		query: q,
	}
}

// WithServiceBinding — see CoreCollector.WithServiceBinding.
func (r *OtelContribCollector) WithServiceBinding(host string, svc *Service) *OtelContribCollector {
	assertNotNil("svc", svc)
	q := r.query.Select("withServiceBinding")
	q = q.Arg("host", host)
	q = q.Arg("svc", svc)

	return &OtelContribCollector{
		query: q,
	}
}

// WithTls — see CoreCollector.WithTls.
func (r *OtelContribCollector) WithTLS(serverCert *File, serverKey *Secret) *OtelContribCollector {
	assertNotNil("serverCert", serverCert)
	assertNotNil("serverKey", serverKey)
	q := r.query.Select("withTls")
	q = q.Arg("serverCert", serverCert)
	q = q.Arg("serverKey", serverKey)

	return &OtelContribCollector{
		query: q,
	}
}

// CoreCollector wraps the otel/opentelemetry-collector image. Its
// public surface is identical to ContribCollector — both share the
// rendering and service-construction helpers below; only the image
// path differs.
type OtelCoreCollector struct { // otel (../../../../:0:0)
	query *querybuilder.Selection

	id               *ID
	otlpGrpcEndpoint *string
	otlpHttpEndpoint *string
	registry         *string
	tag              *string
}
type WithOtelCoreCollectorFunc func(r *OtelCoreCollector) *OtelCoreCollector

// With calls the provided function with current OtelCoreCollector.
//
// This is useful for reusability and readability by not breaking the calling chain.
func (r *OtelCoreCollector) With(f WithOtelCoreCollectorFunc) *OtelCoreCollector {
	return f(r)
}

func (r *OtelCoreCollector) WithGraphQLQuery(q *querybuilder.Selection) *OtelCoreCollector {
	return &OtelCoreCollector{
		query: q,
	}
}

func (r *OtelCoreCollector) BindingHosts(ctx context.Context) ([]string, error) {
	q := r.query.Select("bindingHosts")

	var response []string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *OtelCoreCollector) BindingSvcs(ctx context.Context) ([]Service, error) {
	q := r.query.Select("bindingSvcs")

	q = q.Select("id")

	type bindingSvcs struct {
		Id ID
	}

	convert := func(fields []bindingSvcs) []Service {
		out := []Service{}

		for i := range fields {
			val := Service{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "Service")
			out = append(out, val)
		}

		return out
	}
	var response []bindingSvcs

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

// ConfigFile returns the file that will be mounted as the collector's
// --config: either the caller-supplied override or the
// pipeline-rendered YAML. Inspecting it does not launch the service.
func (r *OtelCoreCollector) ConfigFile() *File {
	q := r.query.Select("configFile")

	return &File{
		query: q,
	}
}

// A unique identifier for this OtelCoreCollector.
func (r *OtelCoreCollector) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *OtelCoreCollector) XXX_GraphQLType() string {
	return "OtelCoreCollector"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *OtelCoreCollector) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *OtelCoreCollector) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *OtelCoreCollector) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *OtelCoreCollector) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = OtelCoreCollector{query: selectNode(dag.query, id, "OtelCoreCollector")}
	return nil
}

// OtlpGrpcEndpoint returns the host:port of the running collector's
// OTLP/gRPC listener (no scheme).
func (r *OtelCoreCollector) OtlpGrpcEndpoint(ctx context.Context) (string, error) {
	if r.otlpGrpcEndpoint != nil {
		return *r.otlpGrpcEndpoint, nil
	}
	q := r.query.Select("otlpGrpcEndpoint")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// OtlpHttpEndpoint returns <scheme>://<host>:4318 for the running
// collector's OTLP/HTTP listener. The scheme is https once WithTls has
// been called, http otherwise.
func (r *OtelCoreCollector) OtlpHTTPEndpoint(ctx context.Context) (string, error) {
	if r.otlpHttpEndpoint != nil {
		return *r.otlpHttpEndpoint, nil
	}
	q := r.query.Select("otlpHttpEndpoint")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *OtelCoreCollector) Override() *File {
	q := r.query.Select("override")

	return &File{
		query: q,
	}
}

func (r *OtelCoreCollector) Pipelines(ctx context.Context) ([]OtelPipeline, error) {
	q := r.query.Select("pipelines")

	q = q.Select("id")

	type pipelines struct {
		Id ID
	}

	convert := func(fields []pipelines) []OtelPipeline {
		out := []OtelPipeline{}

		for i := range fields {
			val := OtelPipeline{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "OtelPipeline")
			out = append(out, val)
		}

		return out
	}
	var response []pipelines

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

func (r *OtelCoreCollector) Registry(ctx context.Context) (string, error) {
	if r.registry != nil {
		return *r.registry, nil
	}
	q := r.query.Select("registry")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// Service returns the running collector. Listens on :4317 (OTLP gRPC)
// and :4318 (OTLP HTTP). Mounts the resolved config (override or
// rendered) when one exists; otherwise launches with no --config flag,
// matching the collector binary's behavior of refusing to start.
func (r *OtelCoreCollector) Service() *Service {
	q := r.query.Select("service")

	return &Service{
		query: q,
	}
}

func (r *OtelCoreCollector) Tag(ctx context.Context) (string, error) {
	if r.tag != nil {
		return *r.tag, nil
	}
	q := r.query.Select("tag")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// WithConfigFile fully replaces the rendered pipeline YAML with the
// supplied file. Pipelines added via WithPipeline are ignored when an
// override is set.
func (r *OtelCoreCollector) WithConfigFile(f *File) *OtelCoreCollector {
	assertNotNil("f", f)
	q := r.query.Select("withConfigFile")
	q = q.Arg("f", f)

	return &OtelCoreCollector{
		query: q,
	}
}

// WithMtls requires client certificates signed by clientCa (PEM-encoded)
// on every incoming OTLP connection. Must be combined with WithTls;
// Service returns an error otherwise.
func (r *OtelCoreCollector) WithMtls(clientCa *File) *OtelCoreCollector {
	assertNotNil("clientCa", clientCa)
	q := r.query.Select("withMtls")
	q = q.Arg("clientCa", clientCa)

	return &OtelCoreCollector{
		query: q,
	}
}

// WithPipeline appends a pipeline to the collector. The collector
// dedupes shared components into one top-level entry per kind/name
// at YAML-render time.
func (r *OtelCoreCollector) WithPipeline(p *OtelPipeline) *OtelCoreCollector {
	assertNotNil("p", p)
	q := r.query.Select("withPipeline")
	q = q.Arg("p", p)

	return &OtelCoreCollector{
		query: q,
	}
}

// WithServiceBinding binds a backend service into the collector's
// network so exporter endpoints can reach it by hostname. Repeated
// calls accumulate.
func (r *OtelCoreCollector) WithServiceBinding(host string, svc *Service) *OtelCoreCollector {
	assertNotNil("svc", svc)
	q := r.query.Select("withServiceBinding")
	q = q.Arg("host", host)
	q = q.Arg("svc", svc)

	return &OtelCoreCollector{
		query: q,
	}
}

// WithTls enables TLS on both OTLP receivers (gRPC :4317 and HTTP :4318).
// serverCert is the PEM-encoded server certificate and serverKey its
// PEM-encoded PKCS#8 private key; both are mounted into the collector and
// wired into every otlp receiver at render time. After this call
// OtlpHttpEndpoint returns an https:// URL.
func (r *OtelCoreCollector) WithTLS(serverCert *File, serverKey *Secret) *OtelCoreCollector {
	assertNotNil("serverCert", serverCert)
	assertNotNil("serverKey", serverKey)
	q := r.query.Select("withTls")
	q = q.Arg("serverCert", serverCert)
	q = q.Arg("serverKey", serverKey)

	return &OtelCoreCollector{
		query: q,
	}
}

// Exporter is a single OpenTelemetry Collector exporter component. When
// TLS options are supplied to the factory, the cert/key material is
// carried here so the collector can mount it at the paths already baked
// into Body (see exporterCertDir).
type OtelExporter struct { // otel (../../../../:0:0)
	query *querybuilder.Selection

	body *string
	id   *ID
	kind *string
	name *string
}

func (r *OtelExporter) WithGraphQLQuery(q *querybuilder.Selection) *OtelExporter {
	return &OtelExporter{
		query: q,
	}
}

func (r *OtelExporter) Body(ctx context.Context) (string, error) {
	if r.body != nil {
		return *r.body, nil
	}
	q := r.query.Select("body")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// A unique identifier for this OtelExporter.
func (r *OtelExporter) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *OtelExporter) XXX_GraphQLType() string {
	return "OtelExporter"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *OtelExporter) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *OtelExporter) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *OtelExporter) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *OtelExporter) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = OtelExporter{query: selectNode(dag.query, id, "OtelExporter")}
	return nil
}

func (r *OtelExporter) Kind(ctx context.Context) (string, error) {
	if r.kind != nil {
		return *r.kind, nil
	}
	q := r.query.Select("kind")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *OtelExporter) Name(ctx context.Context) (string, error) {
	if r.name != nil {
		return *r.name, nil
	}
	q := r.query.Select("name")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// Pipeline is a single OpenTelemetry Collector pipeline binding a
// signal kind to an ordered set of receivers, processors, and
// exporters. Components are held by reference; the collector
// deduplicates shared components into one top-level entry per
// kind/name when the YAML is rendered.
type OtelPipeline struct { // otel (../../../../:0:0)
	query *querybuilder.Selection

	id     *ID
	name   *string
	signal *string
}
type WithOtelPipelineFunc func(r *OtelPipeline) *OtelPipeline

// With calls the provided function with current OtelPipeline.
//
// This is useful for reusability and readability by not breaking the calling chain.
func (r *OtelPipeline) With(f WithOtelPipelineFunc) *OtelPipeline {
	return f(r)
}

func (r *OtelPipeline) WithGraphQLQuery(q *querybuilder.Selection) *OtelPipeline {
	return &OtelPipeline{
		query: q,
	}
}

func (r *OtelPipeline) Exporters(ctx context.Context) ([]OtelExporter, error) {
	q := r.query.Select("exporters")

	q = q.Select("id")

	type exporters struct {
		Id ID
	}

	convert := func(fields []exporters) []OtelExporter {
		out := []OtelExporter{}

		for i := range fields {
			val := OtelExporter{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "OtelExporter")
			out = append(out, val)
		}

		return out
	}
	var response []exporters

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

// A unique identifier for this OtelPipeline.
func (r *OtelPipeline) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *OtelPipeline) XXX_GraphQLType() string {
	return "OtelPipeline"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *OtelPipeline) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *OtelPipeline) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *OtelPipeline) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *OtelPipeline) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = OtelPipeline{query: selectNode(dag.query, id, "OtelPipeline")}
	return nil
}

func (r *OtelPipeline) Name(ctx context.Context) (string, error) {
	if r.name != nil {
		return *r.name, nil
	}
	q := r.query.Select("name")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *OtelPipeline) Processors(ctx context.Context) ([]OtelProcessor, error) {
	q := r.query.Select("processors")

	q = q.Select("id")

	type processors struct {
		Id ID
	}

	convert := func(fields []processors) []OtelProcessor {
		out := []OtelProcessor{}

		for i := range fields {
			val := OtelProcessor{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "OtelProcessor")
			out = append(out, val)
		}

		return out
	}
	var response []processors

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

func (r *OtelPipeline) Receivers(ctx context.Context) ([]OtelReceiver, error) {
	q := r.query.Select("receivers")

	q = q.Select("id")

	type receivers struct {
		Id ID
	}

	convert := func(fields []receivers) []OtelReceiver {
		out := []OtelReceiver{}

		for i := range fields {
			val := OtelReceiver{id: &fields[i].Id}
			val.query = selectNode(q.Root(), fields[i].Id, "OtelReceiver")
			out = append(out, val)
		}

		return out
	}
	var response []receivers

	q = q.Bind(&response)

	err := q.Execute(ctx)
	if err != nil {
		return nil, err
	}

	return convert(response), nil
}

func (r *OtelPipeline) Signal(ctx context.Context) (string, error) {
	if r.signal != nil {
		return *r.signal, nil
	}
	q := r.query.Select("signal")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// WithExporter — see WithReceiver.
func (r *OtelPipeline) WithExporter(exp *OtelExporter) *OtelPipeline {
	assertNotNil("exp", exp)
	q := r.query.Select("withExporter")
	q = q.Arg("exp", exp)

	return &OtelPipeline{
		query: q,
	}
}

// WithProcessor — see WithReceiver.
func (r *OtelPipeline) WithProcessor(proc *OtelProcessor) *OtelPipeline {
	assertNotNil("proc", proc)
	q := r.query.Select("withProcessor")
	q = q.Arg("proc", proc)

	return &OtelPipeline{
		query: q,
	}
}

// WithReceiver appends a receiver to the pipeline and returns a new
// pipeline; the receiver is held by reference so it can be deduped
// across pipelines at render time.
func (r *OtelPipeline) WithReceiver(recv *OtelReceiver) *OtelPipeline {
	assertNotNil("recv", recv)
	q := r.query.Select("withReceiver")
	q = q.Arg("recv", recv)

	return &OtelPipeline{
		query: q,
	}
}

// Processor is a single OpenTelemetry Collector processor component.
type OtelProcessor struct { // otel (../../../../:0:0)
	query *querybuilder.Selection

	body *string
	id   *ID
	kind *string
	name *string
}

func (r *OtelProcessor) WithGraphQLQuery(q *querybuilder.Selection) *OtelProcessor {
	return &OtelProcessor{
		query: q,
	}
}

func (r *OtelProcessor) Body(ctx context.Context) (string, error) {
	if r.body != nil {
		return *r.body, nil
	}
	q := r.query.Select("body")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// A unique identifier for this OtelProcessor.
func (r *OtelProcessor) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *OtelProcessor) XXX_GraphQLType() string {
	return "OtelProcessor"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *OtelProcessor) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *OtelProcessor) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *OtelProcessor) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *OtelProcessor) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = OtelProcessor{query: selectNode(dag.query, id, "OtelProcessor")}
	return nil
}

func (r *OtelProcessor) Kind(ctx context.Context) (string, error) {
	if r.kind != nil {
		return *r.kind, nil
	}
	q := r.query.Select("kind")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *OtelProcessor) Name(ctx context.Context) (string, error) {
	if r.name != nil {
		return *r.name, nil
	}
	q := r.query.Select("name")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// Receiver is a single OpenTelemetry Collector receiver component.
// Body is the YAML body for this component, spliced under
// `receivers.<kind>/<name>` at render time.
type OtelReceiver struct { // otel (../../../../:0:0)
	query *querybuilder.Selection

	body *string
	id   *ID
	kind *string
	name *string
}

func (r *OtelReceiver) WithGraphQLQuery(q *querybuilder.Selection) *OtelReceiver {
	return &OtelReceiver{
		query: q,
	}
}

func (r *OtelReceiver) Body(ctx context.Context) (string, error) {
	if r.body != nil {
		return *r.body, nil
	}
	q := r.query.Select("body")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// A unique identifier for this OtelReceiver.
func (r *OtelReceiver) ID(ctx context.Context) (ID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response ID

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *OtelReceiver) XXX_GraphQLType() string {
	return "OtelReceiver"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *OtelReceiver) XXX_GraphQLIDType() string {
	return "ID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *OtelReceiver) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
	return string(id), nil
}

func (r *OtelReceiver) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
	return json.Marshal(id)
}
func (r *OtelReceiver) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = OtelReceiver{query: selectNode(dag.query, id, "OtelReceiver")}
	return nil
}

func (r *OtelReceiver) Kind(ctx context.Context) (string, error) {
	if r.kind != nil {
		return *r.kind, nil
	}
	q := r.query.Select("kind")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *OtelReceiver) Name(ctx context.Context) (string, error) {
	if r.name != nil {
		return *r.name, nil
	}
	q := r.query.Select("name")

	var response string

	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *Query) Otel() *Otel { // otel (../../../../:0:0)
	q := r.query.Select("otel")

	return &Otel{
		query: q,
	}
}
