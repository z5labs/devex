...
// Format rewrites the configuration with `tofu fmt -recursive` and returns the
...
//
// +cache="session"
func (c *Config) Format(ctx context.Context) (*dagger.Directory, error) {
	if err := c.validate(); err != nil {
		return nil, err
	}
	exec := c.container().WithExec(
		[]string{"tofu", "fmt", "-recursive", "-no-color"},
		dagger.ContainerWithExecOpts{Expect: dagger.ReturnTypeAny},
	)
	if err := expectSuccess(ctx, exec, "tofu fmt -recursive"); err != nil {
		return nil, err
	}
...
// Lock regenerates the dependency lock file for the given platforms with
...
func (c *Config) Lock(
	ctx context.Context,
	// Platforms to record hashes for, as `<os>_<arch>`. Defaults to the
	// platform tofu runs on.
	// +optional
	platforms []string,
	...
	args := []string{"tofu", "providers", "lock", "-no-color"}
...
// withoutCarriedState drops the file-carried state container() wrote into the
// root module. Format and Lock hand that whole directory back, and neither is
// a state operation: a caller exporting the result over their working copy
// should get their configuration, not a state file they never asked for.
func (c *Config) withoutCarriedState(dir *dagger.Directory) *dagger.Directory {
	if c.State == nil {
		return dir
	}
	return dir.WithoutFile(c.statePath())
}
